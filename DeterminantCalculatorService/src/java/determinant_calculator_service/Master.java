...
public class Master extends UntypedActor {
	...
	private void handleAddWorker(Messages.AddWorker rw) {
		...
		System.out
				.println("[" + me + "] " + worker.getRemoteAddress() + " added. Number of workers: " + workers.size());
	}
		...
		for (int i = 0; i < workers.size(); i++) {
			if (workers.get(i).getRemoteAddress().equals(remoteAddress)) {
				...
				while (!jobsToReassign.isEmpty()) {
					...
					workers.get(index)
							.getActorRef()
							.tell(new Messages.Rows(reqId, requestManager.getFirstRow(), requestManager.getRows(nRows,
									rowNumber), rowNumber), getSelf());
				...
				workers.remove(i);
				// System.out.println(workerToRemove.getRemoteAddress() + " removed. Number of workers: " +
				// workers.size());
				return;
			}
		}
	...
	private void sendRowsToWorkers(RequestManager requestManager, String reqId) {
		...
		int maxNRowsPerMsg = MAX_ELEMS_PER_MSG / currentOrder; // MAX_ELEMS_PER_MSG deve essere maggiore dell'ordine
																// massimo possibile
		...
		for (int i = 0; i < (workers.size()); i++) {
			...
			if (nRowsToSend > 0) {
				...
				int nMsgToSend = nRowsToSend / maxNRowsPerMsg;
				...
				if (nRowsOfFirstMsg != 0) {
					...
					workers.get(i)
							.getActorRef()
							.tell(new Messages.Rows(reqId, requestManager.getFirstRow(), requestManager.getRows(
									nRowsOfFirstMsg, nRowsSent + 1), nRowsSent + 1), getSelf());
					// System.out.println(reqId + ", sent rows from " + (nRowsSent + 1) + " to " + (nRowsSent +
					// nRowsOfFirstMsg) + " ("
				...
				for (int j = 0; j < nMsgToSend; j++) {
					...
					workers.get(i)
							.getActorRef()
							.tell(new Messages.Rows(reqId, requestManager.getFirstRow(), requestManager.getRows(
									maxNRowsPerMsg, nRowsSent + 1), nRowsSent + 1), getSelf());
					// System.out.println(reqId + ", sent rows from " + (nRowsSent + 1) + " to " + (nRowsSent +
					// maxNRowsPerMsg) + " ("
	...
	/**
	 * Initializes the Gauss algorithm. If a zero column is detected the computation can stop, the determinant of the
	 * matrix is zero.
	 * 
				...
				while (!jobsToReassign.isEmpty()) {
					...
					for (int k = 0; k < workers.size(); k++) {
						...
						if (!workerIndexNodeAddress.equals(workerNodeFailedAddress)) {
							...
							workers.get(index)
									.getActorRef()
									.tell(new Messages.Rows(reqId, requestManager.getFirstRow(),
											requestManager.getRows(nRows, rowNumber), rowNumber), getSelf());
							System.out.println(reqId + ", reassigned rows from " + rowNumber + " to "
									+ (rowNumber + nRows - 1) + " (" + nRows + " rows) to "
									+ workers.get(index).getRemoteAddress());
